%%
%% This is file `socius_conference_paper_v4.tex',
%%
...
However, the LLM landscape is diverse, and other models present alternatives. Google's Gemini family of models has demonstrated strong performance, particularly in tasks requiring nuanced contextual understanding and sustained conversational coherence. A notable example is the ELEVATE virtual companion, a Gemini-powered mental health support tool for students, which showed iterative differences in contextual understanding and empathetic engagement over five development cycles \cite{Angeline2024}. This demonstrated effectiveness in a supportive, educational context, combined with the architectural flexibility of using both fast (Gemini Flash) and powerful (Gemini Pro) models, made the Gemini API a viable and strategic choice as the foundational LLM for the Socius system's dual-agent architecture.
...
Simultaneously, deploying AI in this domain requires ethical consideration. LLMs can inherit and perpetuate biases from their training data, and the sensitive nature of user conversations necessitates privacy and data security measures \cite{Zhou2024}. Our methodology, which includes a formal ethics review and data anonymization, is designed to mitigate these risks and ensure the responsible development and deployment of Socius.
...
\begin{figure}[H]
\centering
\includegraphics[width=1.00\columnwidth]{figures/[Socius] Architecture.png}
\caption{\small\textit{Socius System Architecture: Dual-Agent Design with AI Partner and AI Mentor}}
\label{fig:architecture}
\end{figure}

Based on our review of current LLMs, the Gemini family was selected for its strong performance in contextual reasoning and its architectural flexibility. The system employs two distinct models to fulfill the roles of the APAM framework:
\begin{itemize}
...
\end{itemize}
The backend manages session state, user profiles, and conversation logs, which are crucial for tracking progress and enabling the holistic mentor functionality.

\subsection{Theoretically-Grounded Scenario Generation}
Our scenarios are not arbitrary conversational prompts but are systematically constructed from a multi-pillar theoretical foundation. This ensures that every interaction emulated by the chatbot is realistic, culturally resonant, and pedagogically sound. Each scenario is synthesized from the following four pillars:

\subsubsection{1. Arnett's Theory of Emerging Adulthood}
Arnett provides the \textbf{psychological context} for our system. He posits that the age range of 18-29 is distinct from adolescence and full adulthood, characterized by identity exploration, instability, self-focus, and feeling "in-between" \cite{Arnett2000}. By grounding scenarios in this theory, we ensure the conflicts feel authentic to the user's current lived experience—specifically the anxiety of navigating undefined roles and the pressure to define one's self.

\subsubsection{2. Havighurst's Developmental Tasks}
Havighurst provides the \textbf{concrete life tasks} or specific social challenges the user must navigate. These include achieving emotional independence from parents, developing new relations with age-mates, and preparing for an economic career \cite{Havighurst1972}. In Socius, these tasks serve as the "plot" of the scenario (e.g., asking for a raise, setting boundaries with a parent), transforming abstract psychological tension into actionable roleplay.

\subsubsection{3. Grice's Cooperative Principle}
Grice's framework provides the \textbf{linguistic challenge} and the mechanism for evaluation. We specifically utilize his four maxims: \textit{Quantity} (amount of information), \textit{Quality} (truthfulness), \textit{Relation} (relevance), and \textit{Manner} (clarity) \cite{Grice1975}. The scenarios are designed to create situations where adhering to these maxims is difficult (e.g., being truthful vs. being polite), forcing the user to navigate the nuance of conversational implicature.

\subsubsection{4. Filipino Cultural Values}
Filipino values provide the \textbf{socio-cultural constraints}, creating the high-context environment that makes pragmatic competence difficult. Following Garcia's analysis \cite{Garcia2013}, we embed values such as \textit{pakikisama} (concession to maintain group harmony), \textit{hiya} (sense of propriety), and \textit{utang na loob} (debt of gratitude). These values often conflict with western ideals of directness (Gricean clarity), creating the central tension the user must resolve.

\subsubsection{Synthesizing Developmental Themes}
The synthesis of these pillars produces four distinct, recurring themes that structure our scenario database:
\begin{enumerate}
    \item \textbf{Identity vs. Expectation:} The collision between a user's emerging self (Arnett) and the life path prescribed by family (Havighurst), testing the balance between Authenticity and Obligation.
    \item \textbf{Autonomy vs. Dependence:} The frustration of being "in-between" (Arnett), where the user seeks adult responsibilities but is treated as a child, highlighting the tension between Independence and Hierarchy.
    \item \textbf{Ambiguity vs. Definition:} Exploring the instability of relationships (Arnett), forcing the user to balance Clarity (Grice) against the Fear of Loss (Pakikisama).
    \item \textbf{Group vs. Self:} The conflict between forming personal values (Havighurst) and peer group norms, testing the user's ability to maintain Personal Integrity within Group Harmony.
\end{enumerate}

\subsection{Pragmatic Competence Scoring Framework}
The cornerstone of our evaluation is a detailed, quantitative scoring framework. After each session, the AI Mentor is prompted to analyze the transcript and return a structured JSON object. This rubric assesses performance across three main categories, comprised of seven specific criteria:
