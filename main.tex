...
\begin{abstract}
Young adults often lack safe, reflective environments to practice nuanced social communication, or pragmatic competence. Existing educational chatbots frequently lack the conversational naturalism or deep, culturally-aware evaluation frameworks necessary for effective training. This paper presents Socius, a conversational AI system for pragmatic competence training built on the AI Partner-AI Mentor (APAM) dual-agent framework. Socius leverages a full Google Gemini architecture, utilizing a low-latency model for immersive roleplaying and a reasoning-enhanced model for deep analysis. A key architectural feature is the system’s multi-pillar theoretical foundation, which grounds practice scenarios in a synthesis of Grice’s Maxims, developmental psychology (Havighurst/Arnett), and explicitly coded Filipino cultural values. We introduce a novel, multi-level scoring framework enforced via JSON Schema Controlled Generation that provides quantitative, actionable metrics on user performance across three core pragmatic domains. Preliminary findings detail the successful implementation of this architecture and report on a crucial experiment: fine-tuning a smaller LLM on a specialized dataset yielded pedagogically poor feedback. This result validated our successful pivot to a sophisticated prompt-engineering approach with a more capable base model, demonstrating the supremacy of reasoning over narrow domain knowledge for complex instructional AI tasks. This paper contributes the architecture and methodology of a theoretically robust, culturally-contextualized system for social skill training.
\end{abstract}
...
Beyond single-session feedback, Socius includes an advanced Holistic Mentor feature, specifically designed to identify long-term patterns and track user progress. This is implemented as a separate session mode (/api/start-holistic-mentor endpoint in \texttt{app.py}) that operates asynchronously from the core practice sessions.

This function retrieves a user’s entire history of performance logs from the Supabase \texttt{conversation\_logs} table. Crucially, rather than processing only raw transcripts, the system aggregates the numerical scores and structured textual feedback generated by the AI Mentor Analyzer from all previous sessions. We leverage the large context window and powerful reasoning of the \texttt{gemini-2.5-pro} model to synthesize this history in a single, high-level prompt.

This allows the Holistic Mentor to generate an opening analysis that identifies recurring strengths (e.g., "consistent high scores on Intention Coherence") and persistent developmental gaps (e.g., "struggles with Formality Level in Theme 1 scenarios, suggesting a challenge with \textit{Paggalang}"). This approach enables the system to provide strategic, longitudinal coaching rather than isolated per-session feedback.

\section{Case Study: The Limits of Fine-Tuning on Small, Specialized Datasets}
Our initial architectural goal for the AI Mentor was to minimize API cost and latency by specializing a smaller, open-source model. We selected \texttt{Mistral-7B} for this task and chose the GRICE dataset \cite{Zheng2021GRICE} for fine-tuning, given its focus on conversational implicature and reasoning.

\subsection{Hypothesis}
We hypothesized that fine-tuning would create a cost-effective, expert model adept at identifying Gricean maxim violations and applying the core pragmatic dimensions of our scoring rubric.

\subsection{Results}
The fine-tuned model performed poorly in a pedagogical context. It demonstrated a critical failure mode: its feedback was overtly literal and brittle, optimized only for pattern matching within the narrow constraints of the dataset. For example, it frequently cited direct violations of the Maxim of Quantity even in socially appropriate scenarios, producing unhelpful advice. Furthermore, it lost its ability to maintain a supportive, encouraging tone, and completely failed to apply the broader theoretical frameworks of Arnett or Filipino cultural values to the "messiness" of the user-generated conversational data. The resulting feedback was consistently evaluated as "direct and unhelpful" across internal testing.

\subsection{Analysis and Architectural Pivot}
We concluded that the GRICE dataset, while invaluable for research on explicit implicature recovery, is too specialized and limited in scope for training a holistic communication coach. The model overfitted to the dataset’s structure, causing it to fail at the complex reasoning and instruction-following required for nuanced pedagogical feedback. This validated our pivot to a prompt-engineered approach utilizing a generalist foundation model (\texttt{gemini-2.5-pro}). By relying on the superior in-context learning and reasoning capabilities of the larger model—guided by our precise system instruction and JSON output schema—we achieved feedback that was immediately superior, contextually relevant, and consistently supportive in tone. This demonstrated the crucial insight that for complex, theory-driven pedagogical tasks, the reasoning capacity of a prompt-tuned generalist LLM significantly outweighs the domain-specific knowledge gained by fine-tuning a smaller model on limited, narrow datasets.

\subsection{Future Work and Remaining Tasks}
