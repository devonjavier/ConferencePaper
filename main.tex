...
\begin{enumerate}
    ...
    \item This paper contributes the architecture and methodology of a theoretically robust, culturally-contextualized system for social skill training.
\end{enumerate}
...
Simultaneously, deploying AI in this domain requires careful ethical consideration. LLMs can inherit and perpetuate biases from their training data, and the sensitive nature of user conversations necessitates robust privacy and data security measures \cite{Zhou2024}. Our methodology, which includes a formal ethics review and data anonymization, is designed to mitigate these risks and ensure the responsible development and deployment of Socius.
...
\subsection{Core Logic: AI Orchestration and User Journey}
The core intelligence involves the orchestration of three distinct AI agents. This design allows us to split the user journey into a "Fast Practice" phase and a "Deep Analysis" phase.
...
\subsection{Linguistic Contextualization}
To ensure the system is culturally resonant for Filipino young adults, we implemented a rigorous set of system instructions within our prompt engineering module (`prompts.py`).
\begin{itemize}
    \item \textbf{Authentic "Taglish":} The AI is explicitly forbidden from providing English translations in parentheses, a common artifact in generic LLMs.
    \item \textbf{Conversational Markers:} The prompt enforces the use of Filipino particles such as \textit{naman}, \textit{nga}, \textit{diba}, and \textit{eh} to soften tone or add emphasis, mirroring natural speech rhythms.
    \item \textbf{Code-Switching for Emotion:} The AI is instructed to switch to Tagalog for words carrying specific emotional weight (e.g., \textit{nakakainis}, \textit{sayang}), reflecting how bilingual Filipinos often revert to their L1 for affective expression.
\end{itemize}
...
The evaluation engine utilizes a detailed, quantitative scoring framework. After each session, the AI Mentor analyzes the transcript and returns a structured JSON object assessing seven specific criteria:
